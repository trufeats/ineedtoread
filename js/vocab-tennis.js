...
function setActiveTeam(i){
  teams.forEach(t => t.el.classList.remove('active'));
  if(i !== null && teams[i]){
    teams[i].el.classList.add('active');
  }
}
...
function reboundScreen(){
  screenOverlay.style.opacity = '';
  screenOverlay.className = 'grey';
  document.body.classList.add('rebound');
  setTimeout(() => {
    screenOverlay.className = '';
    document.body.classList.remove('rebound');
  }, 2000);
}
